...
import { ArrowLeft, Share2, Clock, User, Calendar, ArrowRight, Zap, Brain, TrendingUp, Palette, MessageSquare, Users, Target, Heart } from "lucide-react";
...
export default function BrandIdentityDigitalAge() {
  const structuredData = generateArticleStructuredData({
    headline: "Building Brand Identity in the Digital Age: Your Guide to Lasting Customer Connection",
    ...
    image: "https://images.unsplash.com/photo-1558655146-d09347e92766?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=600",
    publisher: {
      name: "InnofyAI",
      logo: {
          ...
          >
            ...
            >
              {brandElements.map((element, index) => (
                ...
                >
                  <div className={`w-12 h-12 bg-gradient-to-r ${element.color} rounded-full flex items-center justify-center text-white group-hover:scale-110 transition-transform flex-shrink-0`}>
                    {element.icon}
                  </div>
                  <div>
                    <h3 className="text-gray-300 font-bold mb-2">{element.title}</h3>
                    <p className="text-gray-400 text-sm">{element.description}</p>
                    <div className={`w-full h-1 bg-gradient-to-r ${element.color} rounded-full mt-3`}></div>
                  </div>
                </motion.div>
              ))}
            </motion.div>
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          >
            ...
            <motion.p
              ...
              variants={fadeUpVariants}
            >
              Let's create a compelling brand identity that resonates with your audience and drives lasting customer connections.
            </motion.p>
            <motion.button
